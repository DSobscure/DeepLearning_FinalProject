...
def plot_n_reconstruct(origin_img, reconstruct_img, reference_img, n = 8):
    ...
    for i in range(n):
        ...
        plt.imshow(reference_img[i].reshape(84, 84, 4))
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
    plt.show()
...
class GAN():
    def __init__(self):
        ...
        self.cae_loss = tf.reduce_sum(tf.pow(self.cae_output - (self.second_observation - self.first_observation), 2))
    ...
    def build_network(self, x, x2, trainable=True):
        ...
        dconv3_hidden = tf.nn.conv2d_transpose(dconv2_hidden, dconv3_weight, dconv3_output_shape, strides = [1,4,4,1], padding='SAME') + dconv3_bias
        dconv3_hidden2 = tf.nn.conv2d_transpose(dconv2_hidden2, dconv3_weight, dconv3_output_shape, strides = [1,4,4,1], padding='SAME') + dconv3_bias

        return dconv3_hidden, dconv3_hidden2, 1, 1#fc2_hidden, fc2_hidden2

    def update(self, sess, ob0, ob1):
        sess.run(self.optimize_cae, feed_dict={self.first_observation : ob0, self.second_observation: ob1})
    ...
    for episode in range(5000):
        ...
        for t in itertools.count():
            ...
            if total_t % 4 == 0:
                ...
                if total_t % 1000 == 0:
                    print("step %d, CAE loss %g"%(total_t, gan.cae_loss.eval(feed_dict={gan.first_observation: state_batch, gan.second_observation: next_state_batch})))
                    ...
                    if total_t % 10000 == 0:
                        test_size = 10
                        test_reconstruct_img = np.reshape(gan.cae_output.eval(feed_dict = {gan.first_observation: state_batch, gan.second_observation: next_state_batch}), [-1, 84, 84, 4])
                        plot_n_reconstruct(next_state_batch, state_batch + test_reconstruct_img, state_batch)

                    #print(code_converter(gan.cg_code.eval(feed_dict={gan.first_observation: state_batch, gan.second_observation: next_state_batch})))
			    # Update network
