...
class CAE():
    def __init__(self):
        ...
        self.rec_loss = tf.reduce_mean(tf.pow(self.rec_states[0] - (self.original_states[0] + self.rec_states[1]), 2))
        
        self.optimize = tf.train.RMSPropOptimizer(0.00025).minimize(self.rec_loss)  
    ...
    def build_network(self, x, trainable=True):
        ...
        dfc2_weight = tf.Variable(tf.truncated_normal([CODE_SIZE, 11*11*32], stddev = 0.02))
        dfc2_bias = tf.Variable(tf.constant(0.02, shape = [11*11*32]))
        dfc2_hidden_sum = tf.matmul(code_layer, dfc2_weight) + dfc2_bias
