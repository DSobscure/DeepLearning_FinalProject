...
class DQN():
    def __init__(self, gamma, action_number):
        ...
        self.next_state = tf.placeholder(shape=[None, 84, 84, 2], dtype=tf.float32, name='next_state')
    ...
    def build_network(self, x, trainable=True):
        conv1_weight = tf.Variable(tf.truncated_normal([8, 8, 2, 32], stddev = 0.02), trainable = trainable)
